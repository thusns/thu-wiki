# LaTeX and Other Templates

PPT template: https://github.com/atomiechen/THU-PPT-Theme

All templates below can be found and used at https://overleaf.tsinghua.edu.cn/templates/.

## "How to Typeset Papers with LaTeX" Lecture Notes

Since 2015, every April and November, the Tsinghua University Library hosts a special training lecture titled "How to Typeset Papers with LaTeX."

The project is available [here](https://github.com/alick/thulib-latex-talk), and a more recent fork is [here](https://github.com/tuna/thulib-latex-talk).

## ThuThesis

ThuThesis stands for Tsinghua University Thesis LaTeX Template.

This package aims to provide a simple and easy-to-use LaTeX template for Tsinghua University theses, including undergraduate comprehensive thesis training, master's theses, doctoral dissertations, and postdoctoral reports.

The project is available [here](https://github.com/tuna/thuthesis).

## ThuWordThesis

Tsinghua undergraduate thesis Word template.

The project is available [here](https://github.com/qbh16/ThuWordThesis).

[Tsinghua Cloud Drive Download](https://cloud.tsinghua.edu.cn/d/5e264d25ce124695b3de)

## THU-Beamer-Theme

A LaTeX beamer theme template for Tsinghua students.

The project is available [here](https://github.com/Trinkle23897/THU-Beamer-Theme).

The Overleaf template is at: https://www.overleaf.com/latex/templates/thu-beamer-theme/vwnqmzndvwyb — you can click to open directly.

## Tsinghua University Chinese Beamer Template

https://www.overleaf.com/latex/templates/qing-hua-da-xue-zhong-wen-beamer-mo-ban/djcnhxpwhrks

## TsinghuaBeamear

https://www.overleaf.com/latex/templates/tsinghuabeamer/gwchbskgbvrm

## Report Presentation for Tsinghua University

https://www.overleaf.com/latex/templates/report-presentation-for-tsinghua-university/yxcbdvyctybp

## thubeamer

https://github.com/YangLaTeX/thubeamer

## THU coursework Template

LaTeX package for students in Tsinghua University to write coursework more efficiently, also can be used by students from other university.

The project is available [here](https://github.com/zhaofeng-shu33/THU-coursework-template).

## Tsinghua University Modern Physics Experiment Report Template

https://www.overleaf.com/latex/templates/thu-emp-qing-hua-da-xue-jin-dai-wu-li-shi-yan-bao-gao-mo-ban/mbgqcryqhwqd

## THU Letter of Recommendation Template

https://www.overleaf.com/latex/templates/thu-letter-of-recommendation-template/ghjfgfhykprk

## A Simple Tsinghua Letterhead Template

https://www.overleaf.com/latex/templates/a-simple-tsinghua-letterhead-template/zhpywsjswnjs

## CV-tsinghua-template

https://github.com/K-Wu/CV-tsinghua-template

## THU-Exam-LaTeX-Template

https://github.com/VicaYang/THU-Exam-LaTeX-Template

## ilatex

Coursework and lab report templates.

https://github.com/liblaf/ilatex
